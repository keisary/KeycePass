...
> **Version** : 3.1 — Juin 2026 *(Mise à jour : alignement sur le code réel V1.0)*  
...
| **F8** | Consultation des statistiques de présence par le Délégué | 🟡 Important — *Partiel V1* |
...
### 3.1 Dans le Périmètre ✅ (Version 1.0 — code livré)
- Application mobile Android (scanner QR, affichage statut, design glassmorphique)
- Application desktop Windows/Mac (administration, tableau de bord)
- Module shared (logique métier, modèles, réseau)
- Communication réseau local Wi-Fi (port 8080, HTTP REST)
- Génération de QR Codes d'enrôlement et de présence hebdomadaire par classe
- Gestion des séances (création, scan enseignant, clôture)
- Tableau de bord présences en temps réel (rafraîchissement polling)
- Paire appareil-session sécurisée (Device Binding via ANDROID_ID)
- Affichage du statut des étudiants non enrôlés (badge « Non Enrôlé »)
- Export des données via l'historique Desktop

### 3.2 Hors Périmètre V1 ❌ (reporté en V4)
- Application iOS / Web
...
- **Rôle Délégué complet** (consultation stats disponible via API, mais IHM mobile non implémentée — voir Roadmap V4)
- **Synchronisation temps réel WebSocket** (implémenté en polling HTTP)
- **Gestion dynamique des emplois du temps** (voir Roadmap V4)
...
| Acteur | Rôle | Interface | Implémenté en V1 |
|--------|------|-----------|------------------|
| **👨‍🎓 Étudiant** | Présence en cours | Mobile Android | ✅ Scanner QR entrée/sortie, consulter son statut |
| **👨‍🏫 Enseignant** | Tenue et clôture de séance | Mobile Android | ✅ Sélectionner sa séance, clôturer, afficher QR de fin |
| **👥 Délégué de classe** | Consultation des statistiques | Mobile Android | 🔶 API disponible (`GET /api/seance/{id}/stats`), IHM non livrée en V1 |
| **🏢 Administration** | Gestion et contrôle | Desktop (PC) | ✅ Dashboard, QR codes, import Excel, historique |
...
| **EF_06** | Consultation Délégué | L'API serveur expose les statistiques de présence via `GET /api/seance/{id}/stats` (totalPresents, totalRetards, totalAbsents, cloture). L'IHM Mobile Délégué est prévue en V4. |
...
| **Polling HTTP** | Rafraîchissement périodique du dashboard (polling sur la DB locale) | Coroutines + StateFlow |
...
> ⚠️ **Note V3.1** : Les WebSockets initialement prévues ne sont pas implémentées dans la version livrée (V1.0). La communication temps réel est remplacée par un rafraîchissement périodique (polling) du dashboard côté Desktop. Les endpoints ci-dessous correspondent au code réel de `KtorServer.kt`.

### 11.1 Endpoints REST Implémentés

| Méthode | Endpoint | Description | Corps (JSON) |
|---------|----------|-------------|-------------|
| **GET** | `/api/ping` | Health check du serveur | — |
| **POST** | `/api/enrolement` | Lier deviceUuid à un matricule (étudiant **ou** enseignant) | `{"matricule", "deviceUuid"}` |
| **POST** | `/api/scan` | Enregistrer un scan début ou fin | `ScanPayload` (matricule, deviceUuid, seanceId, timestamp, scanType, lat?, lon?) |
| **GET** | `/api/seance/{id}/statut` | Vérifier le statut d'une séance | — |
| **POST** | `/api/seance/{id}/cloturer` | Clôture de séance par l'enseignant | — |
| **GET** | `/api/seance/{id}/stats` | Statistiques de présence (pour le délégué) | — |
...
| **POST** | `/api/sync` | Réception batch de scans hors-ligne | `List<ScanPayload>` |

**Base URL** : `http://{IP_DU_DESKTOP}:8080`  
L'IP est détectée automatiquement au démarrage et affichée dans la console.

### 11.2 Format des QR Codes (tel qu'implémenté)
...
### 12.3 Schéma de la Base de Données Centrale (Desktop - **SQLite**)

> ⚠️ **Note V3.1** : La base de données utilisée est SQLite (via Exposed ORM + sqlite-jdbc), **pas H2**. Le fichier est `desktopApp/database/keycepass_central.db`, versionné dans Git.
> Les types `DATE` et `TIME` sont stockés sous forme de chaînes (`VARCHAR`) formatées `YYYY-MM-DD` et `HH:MM:SS` pour une compatibilité maximale avec Kotlin Multiplatform.
...
## 19. Roadmap V4 — Fonctionnalités Futures

Cette section documente les fonctionnalités **prévues mais non implémentées** dans la version V1.0 du projet, ainsi que les évolutions planifiées pour une version future (V4).

### 19.1 Rôle Délégué — Vérification de la Véracité des Présences

Dans la version V1.0, le rôle `DELEGUE` est défini dans la base de données et dans le `SessionManager` mobile, mais **aucune interface utilisateur dédiée n'a été livrée** (le `DelegueViewModel` est vide, pas d'écran `DelegueScreen`).

**V4 prévoira :**

| Fonctionnalité | Description |
|----------------|-------------|
| **IHM Mobile Délégué** | Écran dédié affichant les statistiques en temps réel de la séance (Présents / Retards / Absents / Non-Enrôlés) via `GET /api/seance/{id}/stats` |
| **Validation physique** | Le délégué peut confirmer visuellement la présence d'étudiants signalés comme « En attente » ou « Absent » en cas de problème technique (perte réseau, scan manqué) |
| **Signalement d'anomalies** | Le délégué peut signaler des irrégularités (étudiant présent physiquement mais non scanné, ou scanné sans être présent) |
| **Envoi du rapport consolidé** | Bouton d'envoi du rapport final vers l'administration via `POST /api/attendance/report` avec validation du délégué |
| **Nouveau endpoint serveur** | `POST /api/attendance/validate-delegue` : permet au délégué d'approuver ou de corriger des statuts individuels, sous réserve d'authentification de son rôle |

### 19.2 Gestion Dynamique des Emplois du Temps

Dans la version V1.0, les séances sont créées manuellement par l'administrateur Desktop (via une interface de saisie journalière). Il n'existe pas de synchronisation avec des calendriers externes ni de modification à la volée depuis l'application mobile.

**V4 prévoira :**

| Fonctionnalité | Description |
|----------------|-------------|
| **Import ICS / EDT** | Chargement automatique des emplois du temps depuis des fichiers iCalendar (`.ics`) ou exports d'ENT |
| **Modification dynamique** | L'administrateur ou l'enseignant peut modifier les horaires d'une séance en direct (report, annulation, changement de salle) depuis le Desktop ou l'app mobile enseignant |
| **Notifications push** | Envoi de notifications aux étudiants en cas de modification de séance (via un système de notification local ou FCM si une connexion Internet est disponible) |
| **Séances exceptionnelles** | Création rapide de séances de rattrapage ou de sessions supplémentaires sans recréer une semaine complète |
| **Synchronisation bidirectionnelle** | Les séances du serveur Desktop sont synchronisées en temps réel avec l'app mobile (remplacement du polling actuel par des WebSockets ou SSE) |
| **Gestion des conflits d'horaires** | Détection et alerte en cas de chevauchement de séances pour une même classe ou un même enseignant |
...
> *Mis à jour V3.1 (Antigravity Agent) — Juin 2026 : alignement sur le code réel V1.0, suppression des WebSockets non implémentées, correction de la DB H2→SQLite, clarification du rôle Délégué, ajout de la Roadmap V4*  
